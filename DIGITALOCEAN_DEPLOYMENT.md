...
## Complete deployment guide for Instify Platform with LiveKit Agents Voice AI
...
# Database
DB_USER=instify_prod
DB_PASSWORD=your_secure_db_password_here
DB_NAME=instify_production
...
- **Customer Context**: Integration with Instify platform data
...
/usr/local/bin/instify-backup

# Manual backup
docker-compose -f docker-compose.prod.yml exec postgres \
  pg_dump -U instify_prod instify_production > backup_$(date +%Y%m%d).sql
```
...
docker-compose -f docker-compose.prod.yml exec -T postgres \
  psql -U instify_prod instify_production < backup_file.sql
...
# View connections
docker-compose -f docker-compose.prod.yml exec postgres \
  psql -U instify_prod -c "SELECT * FROM pg_stat_activity;"
```
...
After successful deployment, your Instify platform will be available at:
