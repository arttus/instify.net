...
export class ODEUOVoiceAgent extends voice.Agent {
  ...
  constructor(
    ...
    const instructions = ODEUOVoiceAgent.buildAgentInstructions(customerContext);
    ...
    agentLogger.info(`🤖 ODEUO Voice Agent initialized for room: ${roomName}`);
  }

  private static buildAgentInstructions(customerContext?: CustomerContext): string {
    const baseInstructions = `
You are an AI assistant for ODEUO, a cutting-edge customer engagement automation platform. 
...
## Core Capabilities:
1. **Platform Guidance**: Help users understand ODEUO's features and capabilities
...
- Keep responses under 100 words unless detailed explanation is requested
    ...
    if (customerContext && Object.keys(customerContext).length > 0) {
      const contextAddition = ODEUOVoiceAgent.formatCustomerContext(customerContext);
  ...
  public getGreetingInstructions(): string {
    if (this.customerContext.company_name) {
      ...
      return `
        Greet the caller warmly and introduce yourself as an AI assistant from ODEUO. 
